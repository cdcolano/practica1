...
public class ControladorVentanaSprites {
	...
	protected static class SpriteSec {  // Datos asociados a cada fotograma de la secuencia
		int zoom = 100;
		int rotacion = 0;
		int offsetX = 0;
		int offsetY = 0;
	...
	public ControladorVentanaSprites( VentanaEdicionSprites vent ) {
		...
		private static ListCellRenderer<File> rendererSprite = (JList<? extends File> list, 
	...
	public void cargaFicherosGraficosOrdenados(File dir) {
		...
		Arrays.sort( fics,((File o1, File o2)-> { return o1.getName().compareTo( o2.getName()); }));
	...
	public void lSecuenciaSelectionChanged() {
		...
		if (fSel!=null) {  // Si hay un elementoseleccionado, mostrarlo en el gráfico
			...
			if (spriteActual>=0) {
				...
				miVentana.slZoom.setValue( ss.zoom ); //TODO sustituir esto por valor de zoom almacenado en la ventana y actualizar el valor de ss zoom
				miVentana.slRotacion.setValue( ss.rotacion ); //igual para rotación
				miVentana.tfoffsetX.setText( "" + ss.offsetX );
				miVentana.tfoffsetY.setText( "" + ss.offsetY );
				miVentana.tfDuracion.setText( "" + ss.duracionMsegs );
			}
